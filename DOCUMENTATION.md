...
- [2026-05-25 13:51:00]: Ottimizzazione UI Mobile pagina Polizze Personali
  - *Details*: Replicato il design "strabiliante" e professionale della pagina Attività sulla pagina delle Polizze Personali. Inoltre, sono stati rimossi i badge inline delle date a favore dei più chiari "Separatori per Data" (in stile timeline) ereditati anch'essi dalla pagina Attività. Infine, per evitare di occupare spazio fisso con il modulo di aggiunta rapida, questo è stato riprogettato come un **Floating Action Button (FAB)** posizionato in basso a destra. Premendo il pulsante "+", si apre un elegante `Dialog` (modale) ottimizzato per il riempimento dei dati in colonna.
  - *Tech Notes*: Modificato il layout in `polizze-personali.tsx`. Il wrapper principale e il componente `Tabs` ora usano `flex flex-col flex-1` per occupare tutta l'altezza. Introdotta logica per estrarre la data, determinare i salti di data tra elementi contigui tramite mappa e renderizzare un separatore testuale. Riscritto `renderQuickAdd`: invece di un `form` "fixed" sul fondo, ora restituisce un `<Dialog>` con un `<DialogTrigger>` configurato come bottone FAB circolare `fixed bottom-24 right-4 md:right-8 w-14 h-14`. Il contenuto del form è stato spostato nel `<DialogContent>` e riorganizzato con una griglia `grid-cols-1 sm:grid-cols-2`.
